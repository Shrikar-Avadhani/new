...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <form id="form1" runat="server">
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:UEBConnectionString16 %>" SelectCommand="SELECT [Name], [Customer_No], [RR_No], [Location_Code], [Meter_No], [Sub_Division], [Prev_Reading], [Issued_Date], [Cur_Reading], [Bill_No], [Due_Date], [Amount] FROM [tblBill] WHERE ([Customer_No] = @Customer_No)"  UpdateCommand ="UPDATE tblBill SET Name = @Name, RR_No = @RR_No, Location_Code = @Location_Code, Meter_No = @Meter_No, Sub_Division = @Sub_Division, Prev_Reading = @Prev_Reading, Cur_Reading=@Cur_Reading, Issued_Date = CONVERT(varchar(10),@Issued_Date,103), Bill_No = ABS(CHECKSUM(NEWID()))%1000 + 1 , Amount = @Amount, Due_Date = CONVERT(varchar(10),@Due_Date,103) WHERE Customer_No = @Customer_No">
            <SelectParameters>
